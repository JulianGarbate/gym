...
import { deleteCardioSession } from "@/app/actions/cardio";

export default function DeleteCardioButton({ id }: { id: string }) {
  ...
  if (confirming) {
    return (
      <div className="flex items-center gap-1.5 rounded-full border border-red-500/40 bg-red-500/10 py-1 pl-3 pr-1.5">
        ...
        <button
          disabled={pending}
          onClick={() => startTransition(() => deleteCardioSession(id))}
  ...
  return (
    <button
      ...
      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-muted transition-colors active:bg-surface-2"
