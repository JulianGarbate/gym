...
import { ChevronRight, ListChecks, Loader2, Zap } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="px-5 py-5 space-y-2.5">
        ...
        >
          <SubmitButton
            className="relative flex min-h-[64px] w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-accent font-semibold text-accent-foreground shadow-[0_8px_30px_-8px] shadow-accent/50 transition-transform active:scale-[0.98] disabled:opacity-70"
            pendingChildren={
              <>
        ...
        {routines.map((r) => (
          ...
          >
            <SubmitButton
              className="flex min-h-[68px] w-full items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-3.5 text-left transition-transform active:scale-[0.98] active:bg-surface-2 disabled:opacity-70"
              pendingChildren={
                <>
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent/10 text-accent">
                    <Loader2 size={18} className="animate-spin" />
                  </div>
                  <p className="flex-1 text-sm text-muted">Iniciando...</p>
                </>
              }
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-accent/10 text-accent">
